...
int virtual_namespace_alloc(struct spd *spd, unsigned long addr, unsigned int size)
{
	...
	unsigned int adj_to = (addr + size-1) >> HPAGE_SHIFT;

	for (i = adj_addr ; i <= adj_to ; i++) {
		if (virtual_spd_layout[i] && virtual_spd_layout[i] != spd) return 0;
	}

	//printk("cos: adding spd %d from %x to %x\n", spd_get_index(spd), addr, addr+size);
	if (addr == 0x4dc00000) printk("cos: adding spd %d from %x to %x\n", spd_get_index(spd), addr, addr+size);
	for (i = adj_addr ; i <= adj_to ; i++) {
...
int spd_add_location(struct spd *spd, long base, long size)
{
	...
	paddr_t pt = spd->spd_info.pg_tbl;
	pgd_t *pgd;
	unsigned long a;
	for (a=0; a<size; a += SERVICE_SIZE) {
		pgd = chal_pgd_lookup(pt, base);
		if (!pgd->pgd) {
			BUG_ON(chal_pgtbl_add_middledir_range(spd->spd_info.pg_tbl, base, size-a));
			BUG_ON(chal_pgtbl_add_middledir_range(spd->composite_spd->pg_tbl, base, size-a));
			break;
		}
		base += SERVICE_SIZE;
	}
