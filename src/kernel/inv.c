			       ...
			       unsigned short int rflags, long *preempt)
{
	...
	*preempt = 0;
	...
	if (unlikely(!da)) {
		...
		printk("kernel thd %d spd %d da %x kerkda %x\n", curr->thread_id, spd_id, da, curr_spd->kern_sched_shared_page[get_cpuid()]);
		goto ret_err;
	}
...
cos_syscall_buff_mgmt_cont(int spd_id, void *addr, unsigned int acap_id, unsigned int len_op)
{
	...
	case COS_BM_XMIT:
	{
		...
		if (likely(cos_net_fns && cos_net_fns->xmit_packet && h)) {
			...
			printk("Kernel send pak err fn %x xmit %x h %x\n", cos_net_fns, cos_net_fns->xmit_packet, h);
		}
		break;
	}
...
ainv_next_thread(struct async_cap *acap, struct thread *preempted, int preempt)
{
	...
	assert(acap && upcall);
	if (upcall->thread_id == 6) {
		mytick++;
		if (mytick > 100*75) {
			struct thread *apa = thd_get_by_id(1);
			upcall = apa;
...
//		printk("cos: split spd with cspd %p from %p, spdid %d\n", trans_cspd, prev, spd1);
		ret = mpd_split(prev, transitory, &sret.cur, &sret.old);
...
cos_syscall_mmap_cntl(int spdid, long op_flags_dspd, vaddr_t daddr, unsigned long mem_id)
{
	...
	case COS_MMAP_GRANT:
		 ...
		 */
		/* ret = chal_pgtbl_add(spd->spd_info.pg_tbl, daddr, page, flags); */
		/* if (ret) { */
		/* 	printk("cos: mmap grant into %d @ %x -- could not add entry to page table. page %x ret %d\n", */
		/* 	       dspd_id, (unsigned int)daddr, page, ret); */
		/* 	ret = -1; */
		/* 	break; */
		/* } */
		...
		chal_pgtbl_add(spd->composite_spd->pg_tbl, daddr, page, flags);
		/* if (chal_pgtbl_add(spd->composite_spd->pg_tbl, daddr, page, flags)) { */
		/* 	printk("cos: mmap grant 2 into %d @ %x -- could not add entry to page table.\n",  */
		/* 	       dspd_id, (unsigned int)daddr); */
		/* 	ret = -1; */
		/* 	break; */
		/* } */
	...
	case COS_MMAP_REVOKE:
	{
		...
		chal_pgtbl_rem(spd->composite_spd->pg_tbl, daddr);
...
cos_syscall_vas_cntl(int id, int op_spdid, long addr, long sz)
{
	...
	op = op_spdid >> 16;
	if (op == 99) {
		struct spd *own = virtual_namespace_query(addr);
		if (own) {
			ret = own-spds;
		}
		return ret;
