...
CCTOR static void posix_init(void);

__attribute__((regparm(1))) long
__cos_syscall(int syscall_num, long a, long b, long c, long d, long e, long f)
{
	/* printc("WARNING: Component %ld calling undifined system call %d %x\n", cos_spd_id(), syscall_num, cos_syscalls[syscall_num]); */
	if (!cos_syscalls[syscall_num]) posix_init();

	assert(syscall_num <= SYSCALLS_NUM);
	if (cos_syscalls[syscall_num] == default_syscall) {
