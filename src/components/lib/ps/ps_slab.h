...
static inline int
ps_slab_hack_nsent(struct ps_slab *s)
{
#define MAX_SENT 150
	struct cbuf_meta *cm;

	cm = cbuf_vect_lookup_addr(s->cbid);
	return cm->snd_rcv.nsent > MAX_SENT;
}
...
__ps_slab_mem_free(void *buf, struct ps_mem *mem, PS_SLAB_PARAMS)
{
	...
	} else if (s->nfree == 1 && !ps_slab_hack_nsent(s)) {
		fl = &mem->percore[coreid].slab_info.fl;
...
__ps_slab_mem_alloc(struct ps_mem *mem, PS_SLAB_PARAMS)
{
	...
	/* remove from the freelist */
	if (s->nfree == 0 || ps_slab_hack_nsent(s)) {
...
ps_slab_cbufid(void *buf, int *off)
{
	struct ps_slab *s;
	struct ps_mheader *h;
