...
static inline void * 
cbuf2buf_p(cbuf_t cb, int len)
{
	struct cbuf_meta *cm;
	void *ret = NULL;
	int id, t;

	if (unlikely(len <= 0)) {printc("cbuf2buf 1 no len\n"); return NULL;}
	cbuf_unpack(cb, &id);
again:
	cm = cbuf_vect_lookup_addr(id);
	if (unlikely(!cm || !CBUF_PTR(cm))) {
		if (__cbuf_2buf_miss(id, len)) {
			printc("cbuf2buf 2 buf miss\n");
			goto done;
		}
		goto again;
	}
	...
	/* shouldn't cbuf2buf your own buffer! */
	assert(cm->cbid_tag.cbid == id);
	if (unlikely(CBUF_OWNER(cm))) {
		printc("cbuf2buf 3 buf no own\n");
		goto done;
	}
	if (unlikely((len >> PAGE_ORDER) > cm->sz)) {
		printc("cbuf2buf 4 buf large len\n");
		goto done;
	}
	assert(!CBUF_TMEM(cm));
