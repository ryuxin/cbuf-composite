...
cos_lock_t cbuf_ring_lock;
#define CBUF_RING_LOCK_INIT() lock_static_init(&cbuf_ring_lock);
#define CBUF_RING_TAKE()      do { if (lock_take(&cbuf_ring_lock))    BUG(); } while(0)
#define CBUF_RING_RELEASE()   do { if (lock_release(&cbuf_ring_lock)) BUG(); } while(0)
...
__cbuf_2buf_miss(int cbid, int len)
{
	...
	if (unlikely(ret < 0 || mc->sz < (len >> PAGE_ORDER))) {
		printc("cbuf miss ret %d spd %d id %d sz %d len %d\n", ret, cos_spd_id(), cbid, mc->sz, len);
		return -1;
	}
	assert(CBUF_PTR(mc));
...
//	assert(cnt++ < 10);
	...
	if (!(ret_cm && CBUF_PTR(ret_cm))) {
		printc("cbuf_c: spd %d cbid %d", cos_spd_id(), ret_cm->cbid_tag.cbid);
	}
