...
int cos_sched_process_events(sched_evt_visitor_t fn, unsigned int proc_amnt)
{
	...
	while (proc_amnt > 0) {
		...
		do {
			...
			assert(!(v_new & 0xFFFF));
		} while (!cos_cas(v_ptr, (unsigned long)v, (unsigned long)v_new));
...
int cos_sched_sync_events(sched_evt_visitor_t fn)
{
	u8_t id, flags;
	u32_t cpu;
	long ret;
	...
	for(i=1; i<18; i++) {
		...
		evt = &PERCPU_GET(cos_sched_notifications)->cos_events[i];
		if (COS_SCHED_EVT_FLAGS(evt) & COS_SCHED_EVT_FREE) continue;
...
void dump_evt(void)
{
	...
	printc("cos sched cur evt %d\n", per_core_sched[cos_cpuid()].cos_curr_evt);
	for(i=1; i<18; i++) {
		evt = &PERCPU_GET(cos_sched_notifications)->cos_events[i];
		f = COS_SCHED_EVT_FLAGS(evt);
		n = COS_SCHED_EVT_NEXT(evt);
		t = sched_evt_to_thd(i);
		if (t) id = t->id;
		else t = 0;
		printc("evt %d thd %d flag %d next %d\n", i, id, f, n);
	}
}
