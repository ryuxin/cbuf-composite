...
static void report_thd_accouting(void)
{
	...
	printc("\n");
	struct sched_thd *myt = sched_get_mapping(14);
	/* dump_evt(); */
	report_output();
...
unsigned long sched_ws_cycles(void)
{
#define THD_NUM 4
	int thdids[THD_NUM] = {11, 12, 13, 14}, i;
	...
	cos_sched_lock_take();
	for(i=0; i<THD_NUM; i++) {
		t = sched_get_mapping(thdids[i]);
		sa = sched_get_accounting(t);
		tot += (sa->new_cyc - sa->prev_cyc);
		sa->prev_cyc = sa->new_cyc;
	}
...
static int sched_switch_thread_target(int flags, report_evt_t evt, struct sched_thd *target)
{
	...
	do {
		...
		if (unlikely(current->id == MYTHD && (current->flags & THD_UC_READY))) {
			printc("cos sched error flag %d\n", current->flags);
			cos_sched_sync_events(evt_callback);
		}
		...
		if(ret < 0) {
			printc("cos sched cur %d switch to %d ret %d\n", current->id, next->id, ret);
			cos_sched_sync_events(evt_callback);
		}
