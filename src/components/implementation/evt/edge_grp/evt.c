...
__evt_free(spdid_t spdid, long eid)
{
	...
	struct evt *temp;	
	if (e->grp) {
		if (e->grp->iachildren == e) {
			if (EMPTY_LIST(e, next, prev)) {
				temp = NULL;
			} else {
				temp = FIRST_LIST(e, next, prev);
			}
			e->grp->iachildren = temp;
		}
		if (e->grp->tchildren == e) {
			if (EMPTY_LIST(e, next, prev)) {
				temp = NULL;
			} else {
				temp = FIRST_LIST(e, next, prev);
			}
			e->grp->tchildren = temp;
		}
	}
...
__evt_trigger(spdid_t spdid, long eid)
{
	...
	for (g = e ; g ; g = g->grp) {
		/* if (g->status & EVT_TRIGGERED) continue; */
		g->status |= EVT_TRIGGERED;
		/* add ourselves to the triggered list of our parent,
		 * and propagate the event up. */
		if (g->grp) {
			struct evt *temp;
			if (g->grp->iachildren == g) {
				if (EMPTY_LIST(g, next, prev)) {
					temp = NULL;
				} else {
					temp = FIRST_LIST(g, next, prev);
				}
				g->grp->iachildren = temp;
			}
			if (g->grp->tchildren == g) {
				if (EMPTY_LIST(g, next, prev)) {
					temp = NULL;
				} else {
					temp = FIRST_LIST(g, next, prev);
				}
				g->grp->tchildren = temp;
			}
