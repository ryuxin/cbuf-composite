...
boot_find_cobjs(struct cobj_header *h, int n)
{
	...
	printc("cobj %s:%d found at %p:%x, size %x -> %x\n",
	       hs[n-1]->name, hs[n-1]->id, hs[n-1], hs[n-1]->size, tot, cobj_sect_get(hs[n-1], 0)->vaddr);
	return total_sz;
}
...
void cos_init(void)
{
	...
	unsigned long sz = (unsigned long)boot_find_cobjs(h, num_cobj);	

	/* This component really might need more vas, as it includes all 
	   components. Furthermore it copies those components again.  */
	void *saddr = cos_get_heap_ptr();

	if (cos_vas_cntl(COS_VAS_SPD_EXPAND, cos_spd_id(),
			 round_up_to_pgd_page((unsigned long)saddr), sz)) {
		printc("Could not expand boot component to %p:%x\n",
		       (void *)round_up_to_pgd_page((unsigned long)saddr), sz);
		BUG();
	}
