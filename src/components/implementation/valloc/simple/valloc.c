...
void *valloc_alloc(spdid_t spdid, spdid_t dest, unsigned long npages)
{
	...
	while (trac->extents[i].map) {
		...
		if (ret == 0x4dc00000) {
			printc("valloc 1 i %d s %x off %d\n", i, trac->extents[i].start, off);
		}
		goto done;
	}
	...
	if (ret == 0x4dc00000) {
		printc("valloc 1 dest %d i %d s %x e %x\n", dest, i, trac->extents[i].start, trac->extents[i].end);
	}
