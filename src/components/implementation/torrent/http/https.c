...
extern int server__treadp(spdid_t spdid, td_t td, int *off, int *sz);
...
PS_SLAB_CREATE_DEF(conn, sizeof(struct connection))
...
struct http_request {
	...
	struct http_request_param *param;
	int cbid, cbsz;
};

PS_SLAB_CREATE_DEF(param, sizeof(struct http_request_param))
PS_SLAB_CREATE_DEF(request, sizeof(struct http_request))

static int http_get_request(struct http_request *r)
{
	int ret = -1, temp, off, sz, cb;
	assert(r && r->c);

	if (0 > r->content_id) {
		r->content_id = server_tsplit(cos_spd_id(), td_root, r->path,
					      r->path_len, TOR_RW, r->c->evt_id);
		if (r->content_id < 0) {
			printc("http mbox split ret %d\n", r->content_id);
			return r->content_id;
		}
		cb = ps_slab_cbufid(r->param, &off);
		sz = sizeof(struct http_request_param);
		temp = (r->content_id << 16) | cb;
		cbuf_send(cb);
		server_twritep(cos_spd_id(), temp, off, sz);
		ret = 0;
	}
	return ret;
}
...
static int http_get(struct http_request *r)
{
	...
	r->param = ps_slab_alloc_param();
	r->param->size = req_sz;
	/* printc("http path %s size %d\n", p, r->param->size); */

	r->path = p;
	r->path_len = strlen(p);/* ret.path_len; */
	r->req_len = (int)(ret.end - s);
...
{
	struct connection *c = (struct connection *)ps_slab_alloc_conn();
...
static inline void http_init_request(struct http_request *r, int buff_len, struct connection *c, char *req)
{
	...
	r->req = req;
	r->req_len = buff_len;
	/* each http connection should only has one request */
...
{
	struct http_request *r = (struct http_request *)ps_slab_alloc_request();
...
static void __http_free_request(struct http_request *r)
{
	...
	ps_slab_free_param(r->param);
	ps_slab_free_request(r);
}
...
connection_handle_request(struct connection *c, char *buff, int cbid, int amnt)
{
	...
	if (http_parse_request(r)) {
		printc("http process fail\n");
	}
	return r;
}

static int connection_parse_requests(struct connection *c, char *req, int cbid, int req_sz)
{
	struct http_request *r;
	
	r = connection_handle_request(c, req, cbid, req_sz);
...
struct head_slab global_head;
static inline void 
head_slab_init(struct head_slab *s, int obj_sz)
{
	s->obj_sz = obj_sz;
	s->mem = cbuf_alloc(PAGE_SIZE, &s->cbid);
	s->nfree = PAGE_SIZE/obj_sz;
}
static inline void *
head_slab_alloc(struct head_slab *s)
{
	if (!s->nfree) {
		cbuf_free(s->cbid);
		head_slab_init(s, s->obj_sz);
	}
	s->nfree--;
	return s->mem + s->nfree*s->obj_sz;
}
static inline int
head_slab_cbinfo(struct head_slab *s, void *m, int *off)
{
	*off = m-s->mem;
	return s->cbid;
}

static int http_get_header(int content_len, int *off, int *sz)
{
	int resp_sz = content_len+1;
	int head_sz = sizeof(success_head)-1;
	int tot_sz, len_sz = 0, ret;
	char len_str[MAX_SUPPORTED_DIGITS], *dest, *head;
	...
	tot_sz = head_sz + len_sz + 1;
	head = dest = (char *)head_slab_alloc(&global_head);
	...
	*sz = tot_sz;
	ret = head_slab_cbinfo(&global_head, head, off);
	return ret;
}
...
static int connection_get_reply(struct connection *c, int *off, int *sz)
{
#define AGGR_SZ 1024
	struct http_request *r;
	struct cbuf_agg *agg;
	int head_id, head_off, head_sz;
	int data_id, data_off, data_sz;
	int ret_id, i, tot=0;
	...
	data_id = server_treadp(cos_spd_id(), r->content_id, &data_off, &data_sz);
	if (data_id <= 0) {
		*sz = data_id;
		printc("https get reply returning %d.\n", *sz);
		return *sz;
	}	
	head_id = http_get_header(data_sz, &head_off, &head_sz);
	if (head_id < 0) printc("too long len %d max %d\n", data_sz, MAX_SUPPORTED_DIGITS);

	agg = (struct cbuf_agg *)cbuf_alloc(AGGR_SZ, &ret_id);
	agg->elem[0].id     = head_id;
	agg->elem[0].offset = head_off;
	agg->elem[0].len    = head_sz;
	agg->elem[1].id     = data_id;
	agg->elem[1].offset = data_off;
	agg->elem[1].len    = data_sz;
	agg->ncbufs = 2;
	cbuf_send(head_id);
...
twrite(spdid_t spdid, td_t td, int cbid, int sz)
{
	...
	lock_connection(c);
	UNLOCK();
	if (connection_parse_requests(c, buf, cbid, sz)) ERR_THROW(-EINVAL, release);
	unlock_connection(c);
	ret = sz;
done:
	cbuf_free(cbid);
...
int 
twritep(spdid_t spdid, td_t temp, int off, int sz)
{
	td_t td;
	int cbid;
	struct connection *c = NULL;
...
treadp(spdid_t spdid, td_t td, int *off, int *sz)
{
	...
	lock_connection(c);
	UNLOCK();
	ret_id = connection_get_reply(c, off, sz);
	unlock_connection(c);
done:	
	return ret_id;
unlock:
...
void report_throughput(void)
{
	printc("http report thd %d\n", cos_get_thd_id());
...
void cos_init(void *arg)
{
	...
	void *vaddr;
	vaddr = valloc_alloc(cos_spd_id(), cos_spd_id(), 80*256);
	assert(vaddr);
	valloc_free(cos_spd_id(), cos_spd_id(), vaddr, 80*256);

	torlib_init();
	lock_static_init(&h_lock);
	head_slab_init(&global_head, sizeof(success_head)+MAX_SUPPORTED_DIGITS);	
	sp.c.type  = SCHEDP_PRIO;
	sp.c.value = 6;

	report_thd = cos_thd_create(report_throughput, NULL, sp.v, 0, 0);
	if (report_thd <= 0) BUG();
