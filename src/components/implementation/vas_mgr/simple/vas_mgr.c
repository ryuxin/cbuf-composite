...
{
	unsigned long a, i;
	printc("vas spd %d start %x end %x amnt %d tot %d\n", dest, start, end, amnt/PAGE_SIZE, ((end-start)/PAGE_SIZE)/256);
	for (a = start, i = start >> PGD_SHIFT; a < end; a += size, i++) {
		...
		*ret_idx = s_idx;
		if (s == 0x4dc00000) {
			printc("vas mgr spd %d dest %x start %x end %x size %d amnt %d nentries %d s %x idx %d\n", spd, dest, start, end, size, amnt, nentries, s, s_idx);
		}
		return s;
	}
	return 0;
...
static void vas_info_sync(void)
{
	unsigned long start = SERVICE_START + (1 << 24);
	unsigned long end = SERVICE_END, size = SERVICE_SIZE;
	unsigned long a, i, r;
	for (a = start, i = start >> PGD_SHIFT; a < end; a += size, i++) {
		r = cos_vas_cntl(99, 0, a, 0);
		if (r) {
			vas->s[i] = vas_get_svi(r);
			/* printc("vas mgr addr %x spd %d\n", a, r); */
		}
	}
}
