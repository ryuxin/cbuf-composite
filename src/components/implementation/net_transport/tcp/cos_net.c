...
{
	char *data = NULL/* (char *)p->alloc_track */;
...
static int __net_bind(spdid_t spdid, net_connection_t nc, struct ip4_addr *ip, u16_t port)
{
	struct intern_connection *ic;
	u16_t tid = cos_get_thd_id();
	int ret = 0;
...
static int __net_connect(spdid_t spdid, net_connection_t nc, struct ip4_addr *ip, u16_t port)
{
	struct intern_connection *ic;
	u16_t tid = cos_get_thd_id();
