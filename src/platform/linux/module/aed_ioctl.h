...
#define AED_SYNC_PGTBL      _IOR(0, 21, unsigned long)
...
static inline int cos_sync_pgtbl(int cntl_fd, struct spd_info *spdi)
{
 	int ret;
	if ((ret = ioctl(cntl_fd, AED_SYNC_PGTBL, spdi))) {
		perror("Could not sync page table\n");
		printf("ioctl returned %d\n", ret);
		exit(-1);
	}
