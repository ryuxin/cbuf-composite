...
static long aed_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AED_SYNC_PGTBL:
	{
		...
		spd = spd_get_by_index(spd_info.spd_handle);
		mm  = aed_get_mm(spd->local_mmaps);
		if (!virtual_namespace_alloc(spd, spd_info.lowest_addr, spd_info.size)) {
			printk("cos: sync pgtbl collision in virtual namespace.\n");
			return -1;
		}

		copy_pgd_range(mm, current->mm, spd_info.lowest_addr, spd_info.size);
		chal_pgtbl_copy_range_nocheck(spd->composite_spd->pg_tbl, spd->spd_info.pg_tbl, 
					 spd_info.lowest_addr, spd_info.size*2);
		return 0;
	}
	default: 
		ret = -EINVAL;
