...
static int obs_serialize(asymbol *symb, void *data)
{
	...
	/* So that we can later add into the exported symbols the user
         * capability table
	 */
	if (symbs->num_symbs >= MAX_SYMBOLS-NUM_KERN_SYMBS-40) {
		printl(PRINT_DEBUG, "Have exceeded the number of allowed "
		       "symbols for object %s.\n", symbs->parent->obj);
		if (!strstr(symb->name, CAP_CLIENT_STUB_POSTPEND) && 
		    strcmp(symb->name, CAP_CLIENT_STUB_DEFAULT) &&
		    !strstr(symb->name, "img_stitch_feature_find") &&
		    !strstr(symb->name, "img_stitch_feature_match") &&
		    !strstr(symb->name, "img_stitch_registration") &&
		    !strstr(symb->name, "img_stitch_warp") &&
		    !strstr(symb->name, "img_stitch_blend"))
		return 0;
	}
...
static void 
sync_pgtbl(int cos_fd, struct service_symbs *boot)
{
	volatile int **heap_ptr;
	struct cos_component_information *ci;
	struct spd_info *spd = boot->extern_info, *spdi;
	heap_ptr = (volatile int **)get_heap_ptr(boot);
	ci       = (void *)get_symb_address(&boot->exported, COMP_INFO);
	spdi     = (struct spd_info *)malloc(sizeof(struct spd_info));
	spdi->lowest_addr = boot->lower_addr;
	spdi->size = (int)*heap_ptr - (int)boot->lower_addr;
	spdi->spd_handle = spd->spd_handle;
	cos_sync_pgtbl(cos_fd, spdi);
}
